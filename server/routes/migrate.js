...
const getChichaPrice = async client => {
  // Source of truth: the Chicha item configured by the admin in Stock.
  // This prevents workers from receiving the old hardcoded 7 DT value.
  const stockResult = await client.query(`
    SELECT price
    FROM items
    WHERE is_tracked = true
      AND (
        LOWER(COALESCE(display_name, '')) LIKE '%chicha%'
        OR LOWER(COALESCE(display_name, '')) LIKE '%shisha%'
        OR LOWER(COALESCE(name, '')) LIKE '%chicha%'
        OR LOWER(COALESCE(name, '')) LIKE '%shisha%'
      )
    ORDER BY id
    LIMIT 1
  `);

  const stockPrice = num(stockResult.rows[0]?.price);
  if (stockPrice > 0) return stockPrice;

  // Legacy fallback for installations where Chicha is not represented as an item.
  await ensureAppSettings(client);
  const r = await client.query(`SELECT value FROM app_settings WHERE key='chicha_price'`);
  const settingsPrice = num(r.rows[0]?.value);
  return settingsPrice > 0 ? settingsPrice : 0;
};
      ...
      const result =
        await transaction(
          async client => {
            ...
            // Expose the admin-configured Chicha price to the worker UI.
